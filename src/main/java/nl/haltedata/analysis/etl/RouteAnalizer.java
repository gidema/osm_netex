...
//    TODO Move internationalization to reporting classes
...
//    List<NetexRouteVariantQuay> netexQuays = netexRouteVariantQuayRepository.
    ...
    private final OsmRouteQuayRepository osmRouteQuayRepository;
    private final NetexRouteVariantQuayRepository netexRouteVariantQuayRepository;
    private final RouteMatchRepository routeMatchRepository;
    private final OsmRouteRepository osmRouteRepository;
    private final NetexRouteVariantRepository netexRouteVariantRepository;
    private final RouteIssueDataRepository routeIssueDataRepository;
    
    public RouteAnalizer(Locale locale, OsmRouteQuayRepository osmRouteQuayRepository,
        NetexRouteVariantQuayRepository netexRouteVariantQuayRepository, RouteMatchRepository routeMatchRepository,
        OsmRouteRepository osmRouteRepository, NetexRouteVariantRepository netexRouteVariantRepository,
        RouteIssueDataRepository routeIssueDataRepository) {
    super();
    this.locale = locale;
    this.osmRouteQuayRepository = osmRouteQuayRepository;
    this.netexRouteVariantQuayRepository = netexRouteVariantQuayRepository;
    this.routeMatchRepository = routeMatchRepository;
    this.osmRouteRepository = osmRouteRepository;
    this.netexRouteVariantRepository = netexRouteVariantRepository;
    this.routeIssueDataRepository = routeIssueDataRepository;
}
    public RouteAnalysis analize(Long routeMatchId) {
        routeMatch = routeMatchRepository.findById(routeMatchId).get();
    ...
    public RouteAnalysis analize(RouteMatch match) {
        ...
        routeIssueDataRepository.deleteAllByRouteMatchId(routeMatch.getId());
        routeIssueDataRepository.saveAll(issues);
        return new RouteAnalysis(locale, routeMatch, osmRoute, issues, quayCountDifference);
    }
...
//                issues.add(new MinorRouteIssue(
...
//            sb.append(i18n.tr("Missing quays found in OSM route between {0} and {1}.:\n", quayBefore, quayAfter));
    ...
    private void checkMatchIssues(QuayMatch match) {
        if (match.isAreaCodeMatch() && ! match.isQuayCodeMatch()) {
            String expected = match.getNetexQuay().getQuayCode();
    ...
    private RouteIssueData addIssue(String message, String... parameters) {
        var issue = new RouteIssueData(issues.size(), routeMatch.getId(),
                message, parameters);
        issues.add(issue);
        return issue;
    }

WITH "match" AS (
SELECT osm_rt.osm_route_id AS osm_route_id, osm_rt.name AS osm_route_name, ntx_rt.name AS netex_route_name, ntx_rt.id AS netex_route_id, ntx_line.name AS netex_line_name
...
WHERE osm_rt.osm_route_id = 15024211
),
quay_idx AS (
SELECT DISTINCT match.osm_route_id, match.netex_route_id, rq.quay_index AS idx
FROM "match"
...
LEFT JOIN osm_pt.osm_route_quay rq ON rq.osm_route_id = match.osm_route_id AND rq.quay_index = quay_idx.idx
LEFT JOIN netex.netex_route_quay nrq ON nrq.route_id = match.netex_route_id AND nrq.quay_index = quay_idx.idx
LEFT JOIN osm_pt.osm_quay ON osm_quay.ref_ifopt = rq.quay_code AND osm_quay.osm_primitive_type = rq.osm_primitive_type
